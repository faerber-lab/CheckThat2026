"""gtr-t5-xl embedding model using sentence-transformers."""
...
class GTRT5XLModel:
    def __init__(self, model_name: str):
        ...
        self.model = SentenceTransformer("gtr-t5-xl", device=self.device)
        self.index = None
    ...
    def index_corpus(self, corpus_texts, corpus_pubkeys, cache_dir=None, batch_size=32):
        """Encode corpus (no prefix needed) and build FAISS index."""
    ...
    def retrieve(self, query_texts, top_k, cache_dir=None, lang=None, batch_size=32):
        """Encode queries (no prefix needed) and search FAISS index."""
        query_embs = self.model.encode(
            query_texts, batch_size=batch_size, show_progress_bar=False,
            normalize_embeddings=True,
        )
